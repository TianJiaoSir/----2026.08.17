...
- 对 AI 给出的结论保持**“辨证心”**，结合**项目的背景**独立判断
...
1. **`getSquareBox` 是四种查询共用的粗筛器**：经度盒子剪一刀、纬度盒子再剪一刀，两轮下来候选集就很小了，不用全表扫
2. **圆形查询的内外方**：先算内接方和外切方，落在内接方里的点不用算距离，直接命中；只有内外方之间那一圈才老老实实算点到圆心的距离
3. **线选为什么要 ±0.001 扩边**：线段本身没有面积，贴线的设备很容易漏，所以查询范围四周各扩一点；至于能不能算命中，由50m的 `RESOLUTION` 容差说了算
4. **拦截器是留给业务条件的口子**：几何判定只管"在不在圈里"，设备类型、查询字符串这些业务过滤交给拦截器，`getDevInAreaList` 里那行 `GisQLUtils.buildInterceptor(query)` 就是在造它
5. **采样放在最后一步**：先把区域内的设备全部捞出来，再按步长抽稀到 sample 个，这样不管大屏圈多大，渲染的点数都可控
